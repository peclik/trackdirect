var trackdirect={services:{},models:{},_time:null,_timetravel:null,_center:null,_zoom:null,_maptype:null,_mid:null,_rulers:[],_filterTimeoutId:null,_waitForFilterResponse:false,_doNotChangeLocationOnFilterResponse:false,_doNotChangeLocationOnFilterResponseTmp:false,_filters:{},_defaultLatitude:null,_defaultLongitude:null,_eventListeners:{},_eventListenersOnce:{},_cordinatesContainerElementId:null,_statusContainerElementId:"td-status-text",_statsContainerElementId:"td-stats-text",_mapElementId:null,_wsServerUrl:null,_map:null,_websocket:null,_mapCreated:false,_trackdirectInitDone:false,isMobile:false,coverageDataUrl:null,coveragePercentile:95,settings:{},init:function(wsServerUrl,mapElementId,options){this._initSettings();this._wsServerUrl=wsServerUrl;this._mapElementId=mapElementId;if($("#"+mapElementId).length<=0){console.log("ERROR: Specified map element missing");return;}
...
var me=this;$.getJSON(this.coverageDataUrl+"?id="+stationId,function(data){if("station_id"in data&&"coverage"in data){coveragePolygon.setData(data["coverage"],me.coveragePercentile);var marker=me._map.markerCollection.getStationLatestMarker(stationId);if(marker.isVisible()){if(coveragePolygon.isRequestedToBeVisible()){coveragePolygon.show();}}}}).fail(function(){coveragePolygon.hide();alert("Failed to fetch coverage data. Try again later!");$("."+coverageLinkElementClass).html("Coverage");}).always(function(){});}}},toggleAllTransmitLinesLink:function(stationId,allTransmitLineLinkElementClass){const map=this._map;const show=!map.markerCollection.hasPermanentTransmitLines(stationId);var anyMatch=false;const lastMarker=map.markerCollection.getStationLatestMarker(stationId);if(lastMarker&&lastMarker.packet.is_moving){for(const mId in map.markerCollection.getStationMarkerIdKeys(stationId)){const m=map.markerCollection.getMarker(mId);if(m!==null){m.toggleTransmitLine(show,true);anyMatch=true;}
for(const dm of map.markerCollection.getDotMarkers(mId)){dm.toggleTransmitLine(show,true);anyMatch=true;}}}else{for(const m of map.markerCollection.getAllMarkers()){if(m.packet.station_id_path.includes(stationId)){m.toggleTransmitLine(show,true);anyMatch=true;}
for(const dm of map.markerCollection.getDotMarkers(m.markerIdKey)){if(dm.packet.station_id_path.includes(stationId)){dm.toggleTransmitLine(show,true);anyMatch=true;}}}}
map.markerCollection.setPermanentTransmitLines(stationId,anyMatch&&show);allTransmitLineLinkElementClass=typeof allTransmitLineLinkElementClass!=="undefined"?allTransmitLineLinkElementClass:null;if(allTransmitLineLinkElementClass!==null){$("."+allTransmitLineLinkElementClass).html((anyMatch&&show)?"Hide Tx-Lines":"All Tx-Lines");}},setMapType:function(mapType){if(this._map!==null){this._map.setMapType(mapType);}},getMapType:function(){if(this._map!==null){return this._map.getMapType();}},setMapDefaultLocation:function(setDefaultZoom){this._map.setMapDefaultLocation(setDefaultZoom);},setMapLocationByGeoLocation:function(failCallBack,successCallBack,timeout){var me=this;if(navigator&&navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){var pos={lat:position.coords.latitude,lng:position.coords.longitude,};me._map.setCenter(pos,12);if(successCallBack!==null){successCallBack();}},function(error){if(failCallBack!==null){failCallBack(error.message);}},{enableHighAccuracy:false,timeout:timeout,maximumAge:5000,});}else{if(failCallBack!==null){failCallBack();}}},openStationInformationDialog:function(stationId){var packet=this._map.markerCollection.getStationLatestPacket(stationId);if(packet==null){packet={station_id:stationId,id:null};}
...
this.trackStation(stationId,stationName,true);}}},_initSettings:function(){this.settings={animate:true,defaultMinZoomForMarkerLabel:10,defaultMinZoomForMarkerPrevPosition:10,defaultMinZoomForMarkerTail:8,defaultMinZoomForMarkers:7,minZoomForMarkerLabel:10,minZoomForMarkerPrevPosition:10,minZoomForMarkerTail:8,minZoomForMarkers:7,markerSymbolBaseDir:"/symbols/",imagesBaseDir:"/images/",defaultCurrentZoom:11,defaultCurrentZoomMobile:11,dateFormat:"L LTSZ",dateFormatNoTimeZone:"L LTS",host:"www.aprsdirect.com",baseUrl:"https://www.aprsdirect.com",defaultTimeLength:60,symbolsToScale:[],primarySymbolWithNoDirectionPolyline:[87,64,95],alternativeSymbolWithNoDirectionPolyline:[40,42,64,74,84,85,96,98,101,102,112,116,119,121,123,],deferred_callbacks:false,remove_old_markers:true,marker_type:2,};},_parseOptions:function(options){if(typeof options["cordinatesContainerElementId"]!==undefined){this._cordinatesContainerElementId=options["cordinatesContainerElementId"];}
if(typeof options["statusContainerElementId"]!==undefined){this._statusContainerElementId=options["statusContainerElementId"];}
if(typeof options["statsContainerElementId"]!==undefined){this._statsContainerElementId=options["statsContainerElementId"];}
...
if(event=="map-created"){this._mapCreated=true;}},_updateStats:function(){if(this._statsContainerElementId!==null){$("#"+this._statsContainerElementId).html(""+this._map.markerCollection._getMarkersCount()+"&nbsp;markers")}},};
...
function layerFactory(L){const CanvasIconLayer=(L.LayerGroup?L.LayerGroup:L.Class).extend({initialize:function(options){L.LayerGroup.prototype.initialize.call(this,[],options);this._onClickListeners=[];this._onHoverListeners=[];this._onMouseOverListeners=[];this._onMouseOutListeners=[];},setOptions:function(options){L.setOptions(this,options);return this.redraw();},addMarkers:function(markers){var self=this;var tmpMark=[];var tmpLatLng=[];markers.forEach(function(marker){if(!((marker.options.pane=='markerPane')&&marker.options.icon)){console.error('Layer isn\'t a marker');return;}
var latlng=marker.getLatLng();var isDisplaying=self._map.getBounds().contains(latlng);var s=self._addMarker(marker,latlng,isDisplaying);if(isDisplaying===true)tmpMark.push(s[0]);tmpLatLng.push(s[1]);});self._markers.load(tmpMark);self._latlngMarkers.load(tmpLatLng);},addMarker:function(marker){const coordinate=marker.getLatLng();const isDisplaying=this._map.getBounds().contains(coordinate);const dat=this._addMarker(marker,coordinate,isDisplaying);if(isDisplaying===true)this._markers.insert(dat[0]);this._latlngMarkers.insert(dat[1]);},addLayer:function(layer){if((layer.options.pane!=='markerPane')){console.log(layer);}
if((layer.options.pane==='markerPane')&&layer.options.icon)this.addMarker(layer);else console.error('Layer isn\'t a marker');},addLayers:function(layers){this.addMarkers(layers);},removeLayer:function(layer){this.removeMarker(layer,true);},layerGroupRemoveLayer:function(layer){var id=layer in this._layers?layer:this.getLayerId(layer);delete this._layers[id];return this;},removeMarker:function(marker,redraw){this.layerGroupRemoveLayer(marker);if(marker["minX"])marker=marker.data;var latlng=marker.getLatLng();var isDisplaying=this._map.getBounds().contains(latlng);var markerData={minX:latlng.lng,minY:latlng.lat,maxX:latlng.lng,maxY:latlng.lat,data:marker};this._latlngMarkers.remove(markerData,function(a,b){return a.data._leaflet_id===b.data._leaflet_id;});this._latlngMarkers.total--;this._latlngMarkers.dirty++;if(isDisplaying===true&&redraw===true){this._redraw(true);}},onAdd:function(map){this._map=map;if(!this._canvas)this._initCanvas();if(this.options.pane)this.getPane().appendChild(this._canvas);else map._panes.overlayPane.appendChild(this._canvas);map.on('moveend',this._reset,this);map.on('resize',this._reset,this);map.on('click',this._executeListeners,this);map.on('mousemove',this._executeListeners,this);},onRemove:function(map){if(this.options.pane)this.getPane().removeChild(this._canvas);else map.getPanes().overlayPane.removeChild(this._canvas);map.off('click',this._executeListeners,this);map.off('mousemove',this._executeListeners,this);map.off('moveend',this._reset,this);map.off('resize',this._reset,this);},addTo:function(map){map.addLayer(this);return this;},clearLayers:function(redraw=true){this._layers={};this._latlngMarkers=null;this._markers=null;this._redraw(redraw);},_closePopup:function(marker){marker.closePopup();},addListener:function(event,func){(this.listeners[event]!=null?this.listeners[event]:(this.listeners[event]=[])).push(func);return this;},layerGroupAddLayer:function(layer){var id=this.getLayerId(layer);this._layers[id]=layer;return this;},_addMarker:function(marker,latlng,isDisplaying){this.layerGroupAddLayer(marker);var self=this;marker._map=self._map;marker.on('move',function(){self._redraw(true);});marker.on('icon-set',function(){marker.canvas_img=null;self._redraw(true);});if(!self._markers)self._markers=new rbush();if(!self._latlngMarkers){self._latlngMarkers=new rbush();self._latlngMarkers.dirty=0;self._latlngMarkers.total=0;}
L.Util.stamp(marker);var pointPos=self._map.latLngToContainerPoint(latlng);var iconSize=marker.options.icon.options.iconSize;var adj_x=iconSize[0]/2;var adj_y=iconSize[1]/2;var ret=[({minX:(pointPos.x-adj_x),minY:(pointPos.y-adj_y),maxX:(pointPos.x+adj_x),maxY:(pointPos.y+adj_y),data:marker}),({minX:latlng.lng,minY:latlng.lat,maxX:latlng.lng,maxY:latlng.lat,data:marker})];self._latlngMarkers.dirty++;self._latlngMarkers.total++;if(isDisplaying===true)self._drawMarker(marker,pointPos);return ret;},_drawMarker:function(marker,pointPos){var self=this;if(!this._imageLookup)this._imageLookup={};if(!pointPos){pointPos=self._map.latLngToContainerPoint(marker.getLatLng());}
if(marker.canvas_img){self._drawImage(marker,pointPos);}else{var iconUrl=marker.options.icon.options.iconUrl;if(self._imageLookup[iconUrl]){marker.canvas_img=self._imageLookup[iconUrl][0];if(self._imageLookup[iconUrl][1]===false){self._imageLookup[iconUrl][2].push([marker,pointPos]);}else{self._drawImage(marker,pointPos);}}else{var i=new Image();i.src=iconUrl;marker.canvas_img=i;self._imageLookup[iconUrl]=[i,false,[[marker,pointPos]]];i.onload=function(){self._imageLookup[iconUrl][1]=true;self._imageLookup[iconUrl][2].forEach(function(e){self._drawImage(e[0],e[1]);});}}}},_drawImage:function(marker,pointPos){const options=marker.options.icon.options;this._context.drawImage(marker.canvas_img,pointPos.x-options.iconAnchor[0],pointPos.y-options.iconAnchor[1],options.iconSize[0],options.iconSize[1]);},_reset:function(){var topLeft=this._map.containerPointToLayerPoint([0,0]);L.DomUtil.setPosition(this._canvas,topLeft);var size=this._map.getSize();this._canvas.width=size.x;this._canvas.height=size.y;this._redraw();},_redraw:function(clear){var self=this;if(!self.redraw_postponed){self.redraw_postponed=true;self.redraw_clear=false;setTimeout(()=>{self.redraw_postponed=false;self._redraw_worker(self.redraw_clear);},150);}
self.redraw_clear|=clear;},_redraw_worker:function(clear){var self=this;if(clear){this._context.clearRect(0,0,this._canvas.width,this._canvas.height);}
if(!this._map||!this._latlngMarkers)
return;var tmp=[];if(self._latlngMarkers.dirty/self._latlngMarkers.total>=.1){self._latlngMarkers.all().forEach(function(e){tmp.push(e);});self._latlngMarkers.clear();self._latlngMarkers.load(tmp);self._latlngMarkers.dirty=0;tmp=[];}
var mapBounds=self._map.getBounds();var mapBoxCoords={minX:mapBounds.getWest(),minY:mapBounds.getSouth(),maxX:mapBounds.getEast(),maxY:mapBounds.getNorth(),};self._latlngMarkers.search(mapBoxCoords).forEach(function(e){var pointPos=self._map.latLngToContainerPoint(e.data.getLatLng());var iconSize=e.data.options.icon.options.iconSize;var adj_x=iconSize[0]/2;var adj_y=iconSize[1]/2;var newCoords={minX:(pointPos.x-adj_x),minY:(pointPos.y-adj_y),maxX:(pointPos.x+adj_x),maxY:(pointPos.y+adj_y),data:e.data}
tmp.push(newCoords);self._drawMarker(e.data,pointPos);});this._markers.clear();this._markers.load(tmp);},_initCanvas:function(){this._canvas=L.DomUtil.create('canvas','leaflet-canvas-icon-layer leaflet-layer');var originProp=L.DomUtil.testProp(['transformOrigin','WebkitTransformOrigin','msTransformOrigin']);this._canvas.style[originProp]='50% 50%';var size=this._map.getSize();this._canvas.width=size.x;this._canvas.height=size.y;this._context=this._canvas.getContext('2d');var animated=this._map.options.zoomAnimation&&L.Browser.any3d;L.DomUtil.addClass(this._canvas,'leaflet-zoom-'+(animated?'animated':'hide'));},addOnClickListener:function(listener){this._onClickListeners.push(listener);},addOnHoverListener:function(listener){this._onHoverListeners.push(listener);},addOnMouseOverListener:function(listener){this._onMouseOverListeners.push(listener);},addOnMouseOutListener:function(listener){this._onMouseOutListeners.push(listener);},_executeListeners:function(event){if(!this._markers)return;var me=this;var x=event.containerPoint.x;var y=event.containerPoint.y;var ret=this._markers.search({minX:x,minY:y,maxX:x,maxY:y});if(ret&&ret.length==0){ret=null;}
if(ret){me._map._container.style.cursor="pointer";if(event.type==="click"){const hasPopup=ret[0].data.getPopup();if(hasPopup)ret[0].data.openPopup();me._onClickListeners.forEach(function(listener){listener(event,ret);});const fakeStop=L.DomEvent.fakeStop||L.DomEvent._fakeStop||L.DomEvent.stop;fakeStop&&fakeStop(event);this._fireEvent([ret[0].data],event,"click");}}else{me._map._container.style.cursor="";}
if(event.type==="mousemove"){if((me._lastRet==null&&ret!=null)||(me._lastRet!=null&&ret==null)||(me._lastRet!=null&&ret!=null&&me._lastRet[0].data._leaflet_id!=ret[0].data._leaflet_id)){if(me._lastRet!=null){if(me._openToolTip){me._openToolTip.closeTooltip();delete me._openToolTip;}
this._fireEvent([me._lastRet[0].data],event,"mouseout");me._onMouseOutListeners.forEach(function(listener){listener(event,me._lastRet);});}
if(ret!=null){var hasTooltip=ret[0].data.getTooltip();if(hasTooltip){me._openToolTip=ret[0].data;ret[0].data.openTooltip();}
this._fireEvent([ret[0].data],event,"mouseover");me._onMouseOverListeners.forEach(function(listener){listener(event,ret);});me._lastRet=ret;}
me._lastRet=ret;}
me._onHoverListeners.forEach(function(listener){listener(event,ret);});}},_fireEvent:function(layers,e,type){if(e.layerPoint&&layers.length>0){layers[0].fire(type||e.type,e,true);return;}
L.Canvas.prototype._fireEvent.call(this,layers,e,type);},});L.canvasIconLayer=function(options){return new CanvasIconLayer(options);};}
...
trackdirect.services.callbackExecutor={settings:{minTimeBeforeSleep:60,},_running:false,_lastSleepTimestamp:0,_lastAddedUniqueCallback:null,_lowPriorityQueue:[],_normalPriorityQueue:[],_highPriorityQueue:[],add:function(thisObj,callback,argsArray){if(!trackdirect.settings.deferred_callbacks){callback.apply(thisObj,argsArray);return;}
var callBackString=callback.toString()+":"+argsArray.toString();this._lastAddedUniqueCallback=callBackString;this._normalPriorityQueue.push(function(){callback.apply(thisObj,argsArray);trackdirect.services.callbackExecutor._next();});this.start();return this;},addIfUnique:function(thisObj,callback,argsArray){if(!trackdirect.settings.deferred_callbacks){callback.apply(thisObj,argsArray);return;}
var callBackString=callback.toString()+":"+argsArray.toString();if(this._lastAddedUniqueCallback==callBackString){return this;}
this._lastAddedUniqueCallback=callBackString;this._normalPriorityQueue.push(function(){callback.apply(thisObj,argsArray);trackdirect.services.callbackExecutor._next();});this.start();return this;},addWithPriority:function(thisObj,callback,argsArray){if(!trackdirect.settings.deferred_callbacks){callback.apply(thisObj,argsArray);return;}
this._highPriorityQueue.push(function(){callback.apply(thisObj,argsArray);trackdirect.services.callbackExecutor._next();});this.start();return this;},addWithLowPriority:function(thisObj,callback,argsArray){if(!trackdirect.settings.deferred_callbacks){callback.apply(thisObj,argsArray);return;}
this._lowPriorityQueue.push(function(){callback.apply(thisObj,argsArray);trackdirect.services.callbackExecutor._next();});this.start();return this;},start:function(){if(!this._running){this._next();}
...
' return false;"><b>'+
relatedStationName+
"</b></a>";rawPath=rawPath.replaceAll("#"+relatedStationName+",","#"+relatedStationNameReplacement+",");rawPath=rawPath.replaceAll("#"+relatedStationName+"*","#"+relatedStationNameReplacement+"*");rawPath=rawPath.replaceAll("#"+relatedStationName+":","#"+relatedStationNameReplacement+":");rawPath=rawPath.replaceAll("#"+relatedStationName+"#","#"+relatedStationNameReplacement+"#");rawPath=rawPath.replaceAll(","+relatedStationName+",",","+relatedStationNameReplacement+",");rawPath=rawPath.replaceAll(","+relatedStationName+"*",","+relatedStationNameReplacement+"*");rawPath=rawPath.replaceAll(","+relatedStationName+":",","+relatedStationNameReplacement+":");rawPath=rawPath.replaceAll(","+relatedStationName+"#",","+relatedStationNameReplacement+"#");}}
...
return rawPath;}
return null;};
(function(){var hasProp={}.hasOwnProperty;null!=this.L&&(this['OverlappingMarkerSpiderfier']=(function(){var _Class,p,twoPi;_Class=class{constructor(map,markerGroup,opts={}){var e,j,k,len,ref,v;this.map=map;this.group=markerGroup;for(k in opts){if(!hasProp.call(opts,k))continue;v=opts[k];(this[k]=v);}
this.initMarkerArrays();this.listeners={};ref=['zoomend'];for(j=0,len=ref.length;j<len;j++){e=ref[j];this.map.addEventListener(e,()=>{return this['unspiderfy']();});}}};p=_Class.prototype;p['VERSION']='0.2.6.1';twoPi=Math.PI*2;p['keepSpiderfied']=false;p['nearbyDistance']=20;p['circleSpiralSwitchover']=9;p['circleFootSeparation']=25;p['circleStartAngle']=twoPi/12;p['spiralFootSeparation']=28;p['spiralLengthStart']=11;p['spiralLengthFactor']=5;p['legWeight']=1.5;p['legColors']={'usual':'#222','highlighted':'#f00'};p.initMarkerArrays=function(){this.markers=[];this.markerListeners=[];};p['addMarker']=function(marker){if(marker['_oms']!=null){return this;}
marker['_oms']=true;var markerListener=()=>{return this.spiderListener(marker);};marker.addEventListener('click',markerListener);this.markerListeners.push(markerListener);this.markers.push(marker);return this;};p['getMarkers']=function(){return this.markers.slice(0);};p['removeMarker']=function(marker){if(marker['_omsData']!=null){this['unspiderfy']();}
this.removeSpiderListeners(marker,null);delete marker['_oms'];this.markers.splice(i,1);return this;};p['clearMarkers']=function(){this['unspiderfy']();var ref=this.markers;for(var i=j=0,len=ref.length;j<len;i=++j){var marker=ref[i];this.removeSpiderListeners(marker,this.markerListeners[i]);delete marker['_oms'];}
...
return this;};p['clearListeners']=function(event){this.listeners[event]=[];return this;};p.trigger=function(event,...args){var func,j,len,ref,ref1,results;ref1=(ref=this.listeners[event])!=null?ref:[];results=[];for(j=0,len=ref1.length;j<len;j++){func=ref1[j];results.push(func(...args));}
return results;};p['removeSpiderListeners']=function(marker,markerListener){if(markerListener==null){var i=this.arrIndexOf(this.markers,marker);if(i>=0){markerListener=this.markerListeners.splice(i,1)[0];}}
if(markerListener!=null){marker.removeEventListener('click',markerListener);}}
p.generatePtsCircle=function(count,centerPt){var angle,angleStep,circumference,i,j,legLength,ref,results;circumference=this['circleFootSeparation']*(2+count);legLength=circumference/twoPi;angleStep=twoPi/count;results=[];for(i=j=0,ref=count;(0<=ref?j<ref:j>ref);i=0<=ref?++j:--j){angle=this['circleStartAngle']+i*angleStep;results.push(new L.Point(centerPt.x+legLength*Math.cos(angle),centerPt.y+legLength*Math.sin(angle)));}
...
return results;};p.findNearbyMarkers=function(marker){var nearbyMarkerData=[];var nonNearbyMarkers=[];var pxSq=this['nearbyDistance']*this['nearbyDistance'];var markerPt=this.map.latLngToLayerPoint(marker.getLatLng());var ref=this.markers;for(var j=0,len=ref.length;j<len;j++){var m=ref[j];if(!this.group.hasLayer(m)){continue;}
var mPt=this.map.latLngToLayerPoint(m.getLatLng());if(this.ptDistanceSq(mPt,markerPt)<pxSq){nearbyMarkerData.push({marker:m,markerPt:mPt});}else{nonNearbyMarkers.push(m);}}
return[nearbyMarkerData,nonNearbyMarkers];}
p.spiderListener=function(marker){var j,len,m,mPt,markerPt,markerSpiderfied,pxSq,ref;markerSpiderfied=marker['_omsData']!=null;if(!(markerSpiderfied&&this['keepSpiderfied'])){this['unspiderfy']();}
if(markerSpiderfied){return this.trigger('click',marker);}else{const[nearbyMarkerData,nonNearbyMarkers]=this.findNearbyMarkers(marker);if(nearbyMarkerData.length===1){return this.trigger('click',marker);}else{return this.spiderfy(nearbyMarkerData,nonNearbyMarkers);}}};p.makeLegHighlightListeners=function(marker){return{highlight:()=>{return marker['_omsData'].leg.setStyle({color:this['legColors']['highlighted']});},unhighlight:()=>{return marker['_omsData'].leg.setStyle({color:this['legColors']['usual']});}};};p.spiderfy=function(markerData,nonNearbyMarkers){var bodyPt,footLl,footPt,footPts,leg,marker,md,mhl,nearestMarkerDatum,numFeet,spiderfiedMarkers;this.spiderfying=true;numFeet=markerData.length;bodyPt=this.ptAverage((function(){var j,len,results;results=[];for(j=0,len=markerData.length;j<len;j++){md=markerData[j];results.push(md.markerPt);}
return results;})());footPts=numFeet>=this['circleSpiralSwitchover']?this.generatePtsSpiral(numFeet,bodyPt).reverse():this.generatePtsCircle(numFeet,bodyPt);spiderfiedMarkers=(function(){var j,len,results;results=[];for(j=0,len=footPts.length;j<len;j++){footPt=footPts[j];footLl=this.map.layerPointToLatLng(footPt);nearestMarkerDatum=this.minExtract(markerData,(md)=>{return this.ptDistanceSq(md.markerPt,footPt);});marker=nearestMarkerDatum.marker;leg=new L.Polyline([marker.getLatLng(),footLl],{color:this['legColors']['usual'],weight:this['legWeight'],clickable:false});this.map.addLayer(leg);marker['_omsData']={usualPosition:marker.getLatLng(),leg:leg};if(this['legColors']['highlighted']!==this['legColors']['usual']){mhl=this.makeLegHighlightListeners(marker);marker['_omsData'].legHighlightListeners=mhl;marker.addEventListener('mouseover',mhl.highlight);marker.addEventListener('mouseout',mhl.unhighlight);}
marker.setLatLng(footLl);if(marker.setZIndexOffset!=null)
marker.setZIndexOffset(1000000);results.push(marker);}
return results;}).call(this);delete this.spiderfying;this.spiderfied=true;return this.trigger('spiderfy',spiderfiedMarkers,nonNearbyMarkers);};p['unspiderfy']=function(markerNotToMove=null){var j,len,marker,mhl,nonNearbyMarkers,ref,unspiderfiedMarkers;if(this.spiderfied==null){return this;}
this.unspiderfying=true;unspiderfiedMarkers=[];nonNearbyMarkers=[];ref=this.markers;for(j=0,len=ref.length;j<len;j++){marker=ref[j];if(marker['_omsData']!=null){this.map.removeLayer(marker['_omsData'].leg);if(marker!==markerNotToMove){marker.setLatLng(marker['_omsData'].usualPosition);}
if(marker.setZIndexOffset!=null)
marker.setZIndexOffset(0);mhl=marker['_omsData'].legHighlightListeners;if(mhl!=null){marker.removeEventListener('mouseover',mhl.highlight);marker.removeEventListener('mouseout',mhl.unhighlight);}
delete marker['_omsData'];unspiderfiedMarkers.push(marker);}else{nonNearbyMarkers.push(marker);}}
delete this.unspiderfying;delete this.spiderfied;this.trigger('unspiderfy',unspiderfiedMarkers,nonNearbyMarkers);return this;};p.ptDistanceSq=function(pt1,pt2){var dx,dy;dx=pt1.x-pt2.x;dy=pt1.y-pt2.y;return dx*dx+dy*dy;};p.ptAverage=function(pts){var j,len,numPts,pt,sumX,sumY;sumX=sumY=0;for(j=0,len=pts.length;j<len;j++){pt=pts[j];sumX+=pt.x;sumY+=pt.y;}
numPts=pts.length;return new L.Point(sumX/numPts,sumY/numPts);};p.minExtract=function(set,func){var bestIndex,bestVal,index,item,j,len,val;for(index=j=0,len=set.length;j<len;index=++j){item=set[index];val=func(item);if((typeof bestIndex==="undefined"||bestIndex===null)||val<bestVal){bestVal=val;bestIndex=index;}}
return set.splice(bestIndex,1)[0];};p.arrIndexOf=function(arr,obj){if(arr.indexOf!=null){return arr.indexOf(obj);}
var a,d,g,f;a=g=0;for(f=arr.length;g<f;a=++g)
if(((d=arr[a]),d===obj))
return a;return-1;};return _Class;})());}.call(this));
...
var marker=new trackdirect.models.Marker(packet,false,this._map);this._addInfoWindowClickListener(marker,true);}else{var marker=new trackdirect.models.Marker(packet,true,this._map);this._map.markerCollection.addDotMarker(markerIdKey,marker);this._addInfoWindowClickListener(marker,true);}
...
var icon=this._getDotMarkerIcon(packet);if(trackdirect.settings.marker_type==0){dotMarker.setOpacity(1.0);}
dotMarker.setIcon(icon);if(typeof google==="object"&&typeof google.maps==="object"){dotMarker.setOptions({anchorPoint:null});}
this._map.markerCollection.addDotMarker(markerIdKey,dotMarker);if(this._map.markerCollection.hasMarkerLabel(markerIdKey)){var markerLabel=this._map.markerCollection.getMarkerLabel(markerIdKey);markerLabel.hide();}
...
marker.packet.map_id=9;if(trackdirect.settings.marker_type==0){dotMarker.setOpacity(0.5);}
marker.hasLabel=false;marker.hideLabel();marker.stopDirectionPolyline();if(!this._map.state.isGhostMarkersVisible){marker.hide();}}}}}};trackdirect.MarkerCreator.prototype._getLatestStationMarkerToConnectTo=function(newPacket,newMarkerIdKey){if(this._map.markerCollection.getStationLatestMovingMarkerIdKey(newPacket.station_id)===newMarkerIdKey){return newMarkerIdKey;}
...
if(packet.db==0){if(this._map.markerCollection.isDuplicate(packet)){}}
if(packet.db==1&&marker.packet.db==1&&marker.packet.source===0&&packet.id===marker.packet.id){console.log("duplicate 1");return true;}
if(marker.overwrite==true&&packet.overwrite==0){return false;}else{if(marker.packet.timestamp>=packet.timestamp){}
if(marker.packet.reported_timestamp!==null&&packet.reported_timestamp!==null&&Math.abs(marker.packet.reported_timestamp-packet.reported_timestamp)<600){if(marker.packet.reported_timestamp>packet.reported_timestamp){}}}}
if(packet.is_moving==1){var list=this._map.markerCollection.getStationMarkerIdKeys(packet.station_id);for(var relatedMarkerIdKey in list){var relatedMarker=this._map.markerCollection.getMarker(relatedMarkerIdKey);if(relatedMarker!==null&&relatedMarker.overwrite==false&&relatedMarker.packet.timestamp>packet.timestamp){}}}
return false;};
trackdirect.models.MarkerCollection=function(){this._markerKeys={};this._markers=[];this._stationMarkers={};this._stationLastMovingMarkerIdKey={};this._stationLastMarker={};this._senderLastMarker={};this._positionMarkersIdKeys={};this._dotMarkers=[];this._markerPolyLines=[];this._markerOriginDashedPolyLines=[];this._mapSectorMarkerIdKeys={};this._stationCoverage={};this._stationPermanentTransmitLines={};this.resetAllMarkers();};trackdirect.models.MarkerCollection.prototype.getMarkerIdKey=function(markerId){if(!(markerId in this._markerKeys)){this._markers.push(null);this._markerPolyLines.push(null);this._dotMarkers.push(null);this._markerOriginDashedPolyLines.push(null);var markerIdKey=this._markers.length-1;this._markerKeys[markerId]=markerIdKey;return markerIdKey;}else{return this._markerKeys[markerId];}};trackdirect.models.MarkerCollection.prototype.isExistingMarker=function(markerIdKey){if(markerIdKey in this._markers&&this._markers[markerIdKey]!==null){return true;}
...
return false;};trackdirect.models.MarkerCollection.prototype.setPermanentTransmitLines=function(stationId,permanentTransmitLines){this._stationPermanentTransmitLines[stationId]=permanentTransmitLines;};trackdirect.models.MarkerCollection.prototype.hasPermanentTransmitLines=function(stationId){if(stationId in this._stationPermanentTransmitLines&&this._stationPermanentTransmitLines[stationId]==true){return true;}
...
return{};};trackdirect.models.Marker.prototype.show=function(){if(typeof google==="object"&&typeof google.maps==="object"){if(typeof this.getMap()==="undefined"||this.getMap()===null){this.setMap(this._defaultMap);}}else if(typeof L==="object"){if(!this._layerGroup.hasLayer(this)){this.addTo(this._layerGroup);}}
...
if(this.isVisible()){if(typeof google==="object"&&typeof google.maps==="object"){this.setMap(null);}else if(typeof L==="object"){this._layerGroup.removeLayer(this);}}
...
if(!this._defaultMap.state.isInternetMarkersVisible&&this.isMovingStation()){if(this.packet.raw_path.indexOf("TCPIP")>=0||this.packet.raw_path.indexOf("qAC")>=0||this.packet.raw_path.indexOf("qAX")>=0||this.packet.raw_path.indexOf("qAU")>=0||this.packet.raw_path.indexOf("qAS")>=0){if(this.packet.station_id_path.length==0){return false;}}}
...
var txTime='';if(this.packet.reported_timestamp!=null)
txTime='<div> tx: '+moment(this.packet.reported_timestamp*1000).format(trackdirect.settings.dateFormatNoTimeZone)+'</div>';var name=escapeHtml(this.packet.station_name);if(this.packet.getOgnRegistration()!=null){name+=", ";name+=escapeHtml(this.packet.getOgnRegistration());if(this.packet.getOgnCN()!==null){name+=" ["+escapeHtml(this.packet.getOgnCN())+"]";}}
...
'</div>');};trackdirect.models.Marker.prototype._getPacketSpeedAltitudeCourseText=function(){if(Math.round(this.packet.speed)!=0||Math.round(this.packet.course)!=0||Math.round(this.packet.altitude)!=0){var text="";if(this.packet.speed!==null){if(this._defaultMap.state.useImperialUnit){text+=Math.round(trackdirect.services.imperialConverter.convertKilometerToMile(this.packet.speed))+" mph ";}else{text+=Math.round(this.packet.speed)+" km/h ";}}
if(this.packet.course!==null){text+=Math.round(this.packet.course)+"&deg; ";}
if(this.packet.altitude!==null){if(this._defaultMap.state.useImperialUnit){text+=" alt "+
Math.round(trackdirect.services.imperialConverter.convertMeterToFeet(this.packet.altitude))+
" ft ";}else{text+=" alt "+Math.round(this.packet.altitude)+" m ";}}
return text;}
return'';};trackdirect.models.Marker.prototype._getTransmitDistanceText=function(){var transmitDistance=this.getTransmitDistance();if(transmitDistance!==null&&Math.round(transmitDistance/100)!=0){var text="";if(this._defaultMap.state.useImperialUnit){text+=Math.round(trackdirect.services.imperialConverter.convertKilometerToMile(transmitDistance/1000)*10)/10+"miles";}else{text+=Math.round(transmitDistance/100)/10+"km";}
return text;}
return'';};trackdirect.models.Marker.prototype._getRxSNRText=function(){const regex=/[-0-9.]+dB/gm;var snr=this.getRxSNR();if(snr!==null)
return snr.toFixed(1)+"dB";return"";};trackdirect.models.Marker.prototype._getGoogleMarkerOptions=function(){var anchorPoint=null;if(this._isDotMarker){var colorId=trackdirect.services.stationColorCalculator.getColorId(this.packet);var iconUrl=trackdirect.settings.baseUrl+
trackdirect.settings.imagesBaseDir+
"dotColor"+
colorId+
...
return{zIndexOffset:this._defaultMap.state.currentMarkerZindex,icon:icon,opacity:opacity,title:tooltipTitle,radius:(trackdirect.settings.marker_type==1)?2:null,};};trackdirect.models.Marker.prototype.setIcon=function(icon){this.options.icon=icon;return this.fire('icon-set',{icon:icon});}
trackdirect.models.Marker.prototype._shouldMarkerSymbolBeScaled=function(){if(typeof trackdirect.settings.symbolsToScale==="undefined"){return false;}
...
return false;};trackdirect.models.Marker.prototype._hideLater=function(delayInMilliSeconds,onlyHideIfNeeded){var me=this;this.hideTimerId=window.setTimeout(function(){if(me._defaultMap.state.isMarkerInfoWindowOpen(me)){me.hide(500);}else{if(!me.shouldMarkerBeVisible()){me.hide(0,onlyHideIfNeeded);}}},delayInMilliSeconds);};trackdirect.models.Marker.prototype._addMarkerTooltip=function(){if(typeof google==="object"&&typeof google.maps==="object"){this._addMarkerGoogleMapTooltip();}else if(typeof L==="object"){this._addMarkerLeafletMapTooltip();}};trackdirect.models.Marker.prototype.toggleTransmitLine=function(show,permanent){if(show===null){if(this.transmitPolyLine!=null&&this.transmitPolyLine.isVisible()){this.transmitPolyLine.hide(0);this.transmitPolyLine.permanent=false;}}else if(show==false){if(this.transmitPolyLine!=null&&this.transmitPolyLine.isVisible()&&(!this.transmitPolyLine.permanent||permanent)){this.transmitPolyLine.hide(show===null?0:2000);this.transmitPolyLine.permanent=false;}}else{if(this.transmitPolyLine==null){this.transmitPolyLine=new trackdirect.models.TransmitPolyline(this.packet,this._defaultMap);this.transmitPolyLine.permanent=false;}
if(!this.transmitPolyLine.permanent)
this.transmitPolyLine.permanent=permanent;this.transmitPolyLine.show();}};trackdirect.models.Marker.prototype._toggleNearbyMarkersTransmitLine=function(show,permanent){var nearbyMarkerData,nonNearbyMarkers;if(this._defaultMap.oms){[nearbyMarkerData,nonNearbyMarkers]=this._defaultMap.oms.findNearbyMarkers(this);}else{nearbyMarkerData=[{marker:this}];}
for(const d of nearbyMarkerData){const m=d.marker;m.toggleTransmitLine(show,permanent);}
};trackdirect.models.Marker.prototype._addMarkerLeafletMapTooltip=function(){if(!trackdirect.isMobile){if(typeof L.tooltip!="undefined"){var tooltip=L.tooltip({direction:"right",noWrap:true,offset:[6,30],className:"leaflet-marker-tooltiptext",});}
...
return null;};trackdirect.models.Marker.prototype.getRxSNR=function(){const regex=/([-0-9.]+)dB/gm;if(this.packet.raw!==null){var m=regex.exec(this.packet.raw);if(m!==null&&m.length>1&&m[1]!==null)
return parseFloat(m[1]);}
return null;};(trackdirect.models.Marker.prototype._addMarkerToOldTimeout=function(altDelayMilliSeconds){if(!trackdirect.settings.remove_old_markers){return;}
var markerCollection=this._defaultMap.markerCollection;var state=this._defaultMap.state;var delay=(this._defaultMap.state.getClientTimestamp(this.packet.timestamp)-
...
trackdirect.models.Map=function(mapElementId,options){this._init(mapElementId,options);this.markerLayer=null;if(typeof google==="object"&&typeof google.maps==="object"){google.maps.Map.call(this,document.getElementById(mapElementId),this._getGoolgeMapOptions());}else if(typeof L==="object"){L.Map.call(this,document.getElementById(mapElementId),this._getLeafletMapOptions());this._updateLeafletTileLayer();L.control.scale({position:"bottomleft",}).addTo(this);L.control.polylineMeasure({showBearings:true,showClearControl:true,showUnitControl:true,unitControlUnits:["kilometres","nauticalmiles"],}).addTo(this);if(trackdirect.settings.marker_type==2){this.markerLayer=L.canvasIconLayer({}).addTo(this);}else{this.markerLayer=this;}}
...
if($(window).height()>=300){var transmitDistance=this._marker.getTransmitDistance();var rxSNR=this._marker.getRxSNR();var tailDistance=this._getTailDistance(this._defaultMap.markerCollection.getMarkerMasterMarkerKeyId(this._marker.markerIdKey));if((transmitDistance!==null&&Math.round(transmitDistance/100)!=0)||rxSNR!=null||(tailDistance!==null&&Math.round(tailDistance)>0)){packetDiv.append(this._getSpaceDiv());packetDiv.append(this._getTransmitDistanceSNRDiv(transmitDistance,rxSNR));packetDiv.append(this._getTailDistanceDiv(tailDistance));}}
...
return packetDiv;};trackdirect.models.InfoWindow.prototype._getPacketTimeDiv=function(){var dateString='';if(this._marker.packet.timestamp==0){dateString+='<span style="color: grey;">No known packet for specified limits.</span>';}else{var txTime='';if(this._marker.packet.reported_timestamp!=null)
dateString+='tx:&nbsp;'+
moment(this._marker.packet.reported_timestamp*1000).format(trackdirect.settings.dateFormatNoTimeZone)+'<br/>';var date=new Date(this._marker.packet.timestamp*1000);dateString+='rx:&nbsp;'+moment(date).format(trackdirect.settings.dateFormatNoTimeZone);if(this._marker.packet.timestamp>this._marker.packet.position_timestamp&&!trackdirect.isMobile){var positionDate=new Date(this._marker.packet.position_timestamp*1000);dateString=moment(positionDate).format(trackdirect.settings.dateFormatNoTimeZone)+
" - "+
moment(date).format(trackdirect.settings.dateFormatNoTimeZone);}
...
return rngDiv;}
return null;};trackdirect.models.InfoWindow.prototype._getTransmitDistanceSNRDiv=function(transmitDistance,rxSNR){if((transmitDistance!==null&&Math.round(transmitDistance/100)!=0)||rxSNR!==null){var transmitDistanceDiv=$(document.createElement("div"));transmitDistanceDiv.css("clear","both");transmitDistanceDiv.css("color","#273c20");if(transmitDistance!==null&&Math.round(transmitDistance/100)!=0){if(this._defaultMap.state.useImperialUnit){transmitDistance=Math.round(trackdirect.services.imperialConverter.convertKilometerToMile(transmitDistance/1000)*10)/10;transmitDistanceUnit="miles";}else{transmitDistance=Math.round(transmitDistance/100)/10;transmitDistanceUnit="km";}
transmitDistanceDiv.append('<span title="Distance to receiving digipeater/igate">Rx dist: '+
transmitDistance+
" "+
transmitDistanceUnit+
"</span>");}
if(rxSNR!==null){transmitDistanceDiv.append(' <span title="Rx signal to noise ration">SNR: '+rxSNR.toFixed(1)+' dB</span>');}
...
if(!trackdirect.isEmbedded&&!inIframe()&&this._marker.packet.source_id!=2){menuUl.append(this._getMenuDivAllTransmitLinesLink());}
...
menuLi.append(menuLink);return menuLi;};trackdirect.models.InfoWindow.prototype._getMenuDivAllTransmitLinesLink=function(){var allTransmitLineLinkElementClass="allTransmitLineLink"+this._marker.packet.station_id;var menuLi=$(document.createElement("li"));menuLi.css(this._getMenuDivLinkCss());var menuLink=$(document.createElement("a"));menuLink.css("color","#337ab7");menuLink.css("white-space","nowrap");menuLink.attr("href","#");menuLink.addClass(allTransmitLineLinkElementClass);menuLink.attr("onclick","trackdirect.toggleAllTransmitLinesLink("+
this._marker.packet.station_id+
', "'+
allTransmitLineLinkElementClass+
'"); return false;');var hasPermanentTransmitLines=this._defaultMap.markerCollection.hasPermanentTransmitLines(this._marker.packet.station_id);if(hasPermanentTransmitLines){menuLink.html("Hide Tx-Lines");}else{menuLink.html("All Tx-Lines");}
