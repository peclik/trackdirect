...
this._map.fitBounds(positions);if(this._map.getZoom()>12){this._map.setZoom(12);}}},_handleAprsPacket:function(packet,animate){if(packet.latitude>=49.183&&packet.latitude<=49.184&&packet.longitude>=17.291&&packet.longitude<=17.292)
console.log("hit 1: "+packet.latitude+"  "+packet.raw_path);var markerIdKey=this._markerCreator.addPacket(packet,true);if(markerIdKey!==null){var highlight=false;var autoRender=false;var marker=this._map.markerCollection.getMarker(markerIdKey);if(animate&&packet.db==0&&packet.station_name==packet.sender_name&&marker.shouldMarkerBeVisible()&&(this.isFilteredMode()||this._isPacketOnMap(packet))){highlight=true;var tdTransmitAnimation=new trackdirect.models.TransmitAnimation(marker,this._map);tdTransmitAnimation.show();}
...
return rawPath;}
return null;};
(function(){var hasProp={}.hasOwnProperty;null!=this.L&&(this['OverlappingMarkerSpiderfier']=(function(){var _Class,p,twoPi;_Class=class{constructor(map,opts={}){var e,j,k,len,ref,v;this.map=map;for(k in opts){if(!hasProp.call(opts,k))continue;v=opts[k];(this[k]=v);}
this.initMarkerArrays();this.listeners={};ref=['click','zoomend'];for(j=0,len=ref.length;j<len;j++){e=ref[j];this.map.addEventListener(e,()=>{return this['unspiderfy']();});}}};p=_Class.prototype;p['VERSION']='0.2.6';twoPi=Math.PI*2;p['keepSpiderfied']=false;p['nearbyDistance']=20;p['circleSpiralSwitchover']=9;p['circleFootSeparation']=25;p['circleStartAngle']=twoPi/12;p['spiralFootSeparation']=28;p['spiralLengthStart']=11;p['spiralLengthFactor']=5;p['legWeight']=1.5;p['legColors']={'usual':'#222','highlighted':'#f00'};p.initMarkerArrays=function(){this.markers=[];this.markerListeners=[];};p['addMarker']=function(marker){if(marker['_oms']!=null){return this;}
...
return results;};p.findNearbyMarkers=function(marker){var nearbyMarkerData=[];var nonNearbyMarkers=[];var pxSq=this['nearbyDistance']*this['nearbyDistance'];var markerPt=this.map.latLngToLayerPoint(marker.getLatLng());var ref=this.markers;for(var j=0,len=ref.length;j<len;j++){var m=ref[j];if(!this.map.hasLayer(m)){continue;}
...
marker.setLatLng(footLl);marker.setZIndexOffset(1000000);results.push(marker);}
...
var markerIdKey=this._map.markerCollection.getMarkerIdKey(packet.marker_id);if(packet.latitude>=49.183&&packet.latitude<=49.184&&packet.longitude>=17.291&&packet.longitude<=17.292){console.log("hit 2: "+packet.latitude+"  "+packet.raw_path);console.log(packet);}
if(this._map.markerCollection.isExistingMarker(markerIdKey)){var marker=this._map.markerCollection.getMarker(markerIdKey);if(marker.packet.is_moving==1&&packet.is_moving!=1){packet.is_moving=1;}}else{if(packet.packet_order_id==2){packet.packet_order_id=3;}}
...
this.transmitPolyLine.permanent=permanent;this.transmitPolyLine.show();}};trackdirect.models.Marker.prototype._toggleNearbyMarkersTransmitLine=function(show,permanent){var nearbyMarkerData,nonNearbyMarkers;if(this._map.oms){[nearbyMarkerData,nonNearbyMarkers]=this._map.oms.findNearbyMarkers(this);}else{nearbyMarkerData.marker.this;}
...
return null;};(trackdirect.models.Marker.prototype._addMarkerToOldTimeout=function(altDelayMilliSeconds){var markerCollection=this._defaultMap.markerCollection;var state=this._defaultMap.state;var delay=(this._defaultMap.state.getClientTimestamp(this.packet.timestamp)-
